...
private fun solution2() :Int {
    var basinPoints = mutableListOf<Pair<Int, Int>>()
    for (rowIdx in heights.indices) {
        val row = heights[rowIdx]
        ...
        for (idx in row.indices) {
            ...
            basinPoints.add(Pair<Int, Int>(idx, rowIdx))
        }
    }

    var basinSizes = mutableListOf<Int>()
    for (point in basinPoints) {
        var basinBounds = mutableSetOf<Pair<Int, Int>>(point)
...
private fun checkForBounds(p: Pair<Int, Int>, basinPoints: Set<Pair<Int, Int>>) :Set<Pair<Int, Int>> {
    ...
    // compare to the point to the left - if it's not 9, and not already in points, add it to points and recurse
    ...
    // compare to the point to the right - if it's not 9, and not already in points, add it to points and recurse
    ...
    // compare to the point above - if it's not 9, and not already in points, add it to points and recurse
